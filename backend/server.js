    ...
    db.serialize(() => {
      ...
      db.run(`
        CREATE TABLE IF NOT EXISTS accounts (
          ...
          diff REAL DEFAULT 0,
          includeInOnTrack INTEGER DEFAULT 1,
          FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
          FOREIGN KEY (version_id) REFERENCES budget_versions(id) ON DELETE CASCADE
        )
      `);
      
      // Add includeInOnTrack column to existing accounts table if it doesn't exist
      db.all("PRAGMA table_info(accounts)", (err, columns) => {
        if (!err && columns && !columns.some(col => col.name === 'includeInOnTrack')) {
          db.run('ALTER TABLE accounts ADD COLUMN includeInOnTrack INTEGER DEFAULT 1');
        }
      });
      ...
      // Create manual_adjustments table for manual account adjustments
      db.run(`
...
// Ensure manual_adjustments table has a description column
// Add column if it does not exist
const ensureManualAdjustmentsDescriptionColumn = () => {
  db.all("PRAGMA table_info(manual_adjustments)", (err, columns) => {
    if (err) {
      console.log('Error checking manual_adjustments table structure:', err);
      return;
    }
    if (!columns || columns.length === 0) {
      console.log('manual_adjustments table does not exist yet');
      return;
    }
    const hasDescription = columns.some(col => col.name === 'description');
    if (!hasDescription) {
      console.log('Adding description column to manual_adjustments table');
      db.run("ALTER TABLE manual_adjustments ADD COLUMN description TEXT DEFAULT ''", (err) => {
        if (err) {
          console.log('Error adding description column:', err);
        } else {
          console.log('Successfully added description column to manual_adjustments table');
        }
      });
    } else {
      console.log('Description column already exists in manual_adjustments table');
    }
  });
};
    ...
    db.run(
      ...
      function(err) {
        ...
        sampleData.accounts.forEach(account => {
          db.run(
            'INSERT INTO accounts (user_id, version_id, name, bank, currentBalance, requiredBalance, isPrimary, diff, includeInOnTrack) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [userId, versionId, account.name, account.bank, account.currentBalance, account.requiredBalance, account.isPrimary, account.diff, account.includeInOnTrack !== false ? 1 : 0]
          );
        });
...
app.get('/api/accounts', authenticateToken, (req, res) => {
  ...
  `, [req.user.userId], (err, rows) => {
    ...
    const accounts = rows.map(row => ({
      ...
      includeInOnTrack: Boolean(row.includeInOnTrack)
    }));
    res.json(accounts);
  });
});
...
app.post('/api/accounts', authenticateToken, (req, res) => {
  const { name, bank, currentBalance, requiredBalance, isPrimary, diff, includeInOnTrack } = req.body;
  ...
  db.run(
      'INSERT INTO accounts (user_id, version_id, name, bank, currentBalance, requiredBalance, isPrimary, diff, includeInOnTrack) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [req.user.userId, version.id, name, bank, currentBalance, requiredBalance, isPrimary ? 1 : 0, diff || 0, includeInOnTrack !== false ? 1 : 0],
    function(err) {
      if (err) {
...
app.put('/api/accounts/:id', authenticateToken, (req, res) => {
  ...
  `, [id, req.user.userId], (err, account) => {
    ...
    db.run(`
      UPDATE accounts SET name = ?, bank = ?, currentBalance = ?, requiredBalance = ?, isPrimary = ?, diff = ?, includeInOnTrack = ? 
      WHERE id = ? AND user_id = ? AND version_id IN (
        SELECT id FROM budget_versions WHERE user_id = ? AND is_active = 1
      )
    `, [name, bank, currentBalance, requiredBalance, isPrimary ? 1 : 0, diff || 0, includeInOnTrack !== false ? 1 : 0, id, req.user.userId, req.user.userId],
    function(err) {
      if (err) {
  ...
  getVersionId((versionId, versionName) => {
    ...
    db.all('SELECT * FROM accounts WHERE user_id = ? AND version_id = ?', [userId, versionId], (err, accounts) => {
      if (!err) {
        ...
        exportData.accounts = accounts.map(account => ({
          ...account,
          isPrimary: Boolean(account.isPrimary),
          includeInOnTrack: Boolean(account.includeInOnTrack)
        }));
      }
      // Export income
      ...
      getUniqueVersionName(baseName, (uniqueName) => {
        ...
        db.run(
          ...
          function(err) {
            ...
            // --- IMPORT: Add after insertExpenseTags() ---
            function insertManualAdjustments() {
              return new Promise((resolve) => {
                if (importData.manualAdjustments && importData.manualAdjustments.length > 0) {
                  let processed = 0;
                  importData.manualAdjustments.forEach(adj => {
                    // Map account_id to new account ID
                    const newAccountId = accountIdMapping.get(adj.account_id) || null;
                    if (!newAccountId) {
                      processed++;
                      if (processed === importData.manualAdjustments.length) resolve();
                      return;
                    }
                    db.run(
                      'INSERT INTO manual_adjustments (user_id, version_id, account_id, type, amount, date, description) VALUES (?, ?, ?, ?, ?, ?, ?)',
                      [userId, versionId, newAccountId, adj.type, adj.amount, adj.date, adj.description],
                      function(err) {
                        processed++;
                        if (processed === importData.manualAdjustments.length) resolve();
                      }
...
app.get('/api/manual-adjustments', authenticateToken, (req, res) => {
  db.get('SELECT id FROM budget_versions WHERE user_id = ? AND is_active = 1', [req.user.userId], (err, version) => {
    if (err || !version) {
      return res.status(400).json({ error: 'No active version found.' });
    }
    db.all(
      'SELECT * FROM manual_adjustments WHERE user_id = ? AND version_id = ? ORDER BY date',
      [req.user.userId, version.id],
      (err, rows) => {
        if (err) return res.status(500).json({ error: err.message });
        res.json(rows);
      }
...
app.post('/api/manual-adjustments', authenticateToken, (req, res) => {
  const { account_id, type, amount, date, description = '' } = req.body;
  if (!account_id || !['withdrawal', 'deposit'].includes(type) || typeof amount !== 'number' || !date) {
  ...
  db.get('SELECT id FROM budget_versions WHERE user_id = ? AND is_active = 1', [req.user.userId], (err, version) => {
    ...
    db.run(
      'INSERT INTO manual_adjustments (user_id, version_id, account_id, type, amount, date, description) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [req.user.userId, version.id, account_id, type, amount, date, description],
      function(err) {
        if (err) return res.status(500).json({ error: err.message });
        res.json({ id: this.lastID });
      }
